...
func GetCmdQueryDenomTrace() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query ibc-exchange denom-trace 27A6394C3F9FF9C9DCF5DFFADF9BB5FE9A37C7E92B006199894CF1824DF9AC7C", version.AppName),
...
func GetCmdParams() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "params",
		Short:   "Query the current ibc-exchange parameters",
		Long:    "Query the current ibc-exchange parameters",
		Args:    cobra.NoArgs,
		Example: fmt.Sprintf("%s query ibc-exchange params", version.AppName),
		RunE: func(cmd *cobra.Command, _ []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
				return err
...
func GetCmdQueryEscrowAddress() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query ibc-exchange escrow-address [port] [channel-id]", version.AppName),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
				return err
...
func GetCmdQueryDenomHash() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query ibc-exchange denom-hash exchange/channel-0/uatom", version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
